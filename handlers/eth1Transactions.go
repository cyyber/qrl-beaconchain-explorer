		...
		for i := len(t) - 1; i >= 0; i-- {
			...
			wg.Go(func() error {
				...
				tableData = append(tableData, []interface{}{
					...
					utils.FormatAmountFormatted(new(big.Int).SetBytes(v.GetValue()), "Quanta", 8, 4, true, true, false),
					utils.FormatAmountFormatted(db.CalculateTxFeeFromTransaction(v, new(big.Int).SetBytes(b.GetBaseFee())), "Quanta", 8, 4, true, true, false),
				})
				return nil
			})
			wg.Wait()
