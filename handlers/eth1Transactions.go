		...
		for i := len(t) - 1; i >= 0; i-- {
			...
			wg.Go(func() error {
				...
				tableData = append(tableData, []interface{}{
					...
					utils.FormatAmountFormatted(new(big.Int).SetBytes(v.GetValue()), "Zond", 8, 4, true, true, false),
					utils.FormatAmountFormatted(db.CalculateTxFeeFromTransaction(v, new(big.Int).SetBytes(b.GetBaseFee())), "Zond", 8, 4, true, true, false),
				})
				return nil
			})
			wg.Wait()
