...
func main() {
	...
	traceMode := flag.String("blocks.tracemode", "gzond", "Trace mode to use, can be 'gzond'")
...
func IndexFromNode(bt *db.Bigtable, client *rpc.GzondClient, start, end, concurrency int64, traceMode string) error {
	...
	for i := start; i <= end; i++ {
		...
		g.Go(func() error {
			...
			if err != nil {
				return fmt.Errorf("error getting block: %v from zond execution node err: %w", i, err)
			}
